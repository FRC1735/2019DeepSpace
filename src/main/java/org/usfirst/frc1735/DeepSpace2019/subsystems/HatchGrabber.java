...
import org.usfirst.frc1735.DeepSpace2019.commands.HatchGrabOpenMove;
import org.usfirst.frc1735.DeepSpace2019.commands.ResetHatchEnc;
...
public class HatchGrabber extends Subsystem {
    ...
    private static final double kMaxGrabValue = 485000; // encoder val for max grab extension (Safety)
    ...
    public void periodic() {
        ...
        // }
        m_launchPadJoystick.updateAlienLightState();
    }
    ...
    public void initHatchGrabber() {
        ...
        if (true /* !Robot.isPracticeBot() */) {
            ...
            hatchGrabberMotor.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative, 0, 0); // extra args
                                                                                                           // are:
                                                                                                           // primary
                                                                                                           // closed
                                                                                                           // loop,
            ...
            // Set the closed-loop allowable error. Empirically on no-load, error was x
            // units.
            hatchGrabberMotor.configAllowableClosedloopError(0, (int) kToleranceDistUnits, 0); // index, err, timeout in
                                                                                               // ms
            ...
            // This does imply that the alien hatchgrabber must be in the stowed/released
            // position upon robot power-on!
            // if not, then the current position will become "stowed" and Bad Things will
            // happen.
            ...
            m_alienTab = Shuffleboard.getTab("Alien"); // Creates tab if it doesn't
            // already exist.

            m_hatchGrabberEncoderEntry = m_alienTab.add("Hatch Enc", 0).withSize(2, 1)
            ...
            m_hatchGrabberMotorOpenMagdirEntry = m_alienTab.add("Hatch Motor", 0).withSize(8, 1) // make the widget 10x1
                                                                                                 // .withPosition(2, 0)
                                                                                                 // // place it in the
                                                                                                 // top-left corner
                    .withWidget(BuiltInWidgets.kNumberSlider).withProperties(Map.of("min", -0.3, "max", 0.3)) // specify
                                                                                                              // widget
                                                                                                              // properties
                                                                                                              // here
                    .getEntry();
            ...
            m_alienTab.add("HatchGrabber Open Loop Run", new HatchGrabOpenMove()).withSize(4, 1).withPosition(0, 4);

            // Command to reset the hatchGrabber's encoder directly from the Shuffleboard
            m_alienTab.add("Reset Hatch Encoder", new ResetHatchEnc()).withSize(6, 1).withPosition(0, 2);

            m_alienTab.add("Force Hatch Encoder to MAX", new ForceHatchEncToMax()).withSize(6, 1).withPosition(8, 2);
    ...
    // *******************************************
    // This function call is intended to get us "out of jail" in case the
    // hatchGrabber's encoder ever needs to be reset manually to the fully closed
    // encoder value.
    // Doing so with the hatch in any other opened position will result in damage to
    // the grabber because we would open too far and break something.
    // The intended use of this function is to reset the encoder on startup in rare
    // cases where it needs to be reset without a full reboot of the robot... like
    // for taring the position.
    // Originally the subsystem init routine called this, but that was occasionally
    // causing a reset of the encoder when the hatchgrabber was still fully
    // extended, so the call has been commented out.
    ...
    // BE EXTREMELY CAREFUL WITH THIS FUNCTION!!!!
    // *******************************************
    // This function call is intended to get us "out of jail" in case the robot is
    // ever shut down or reset while the hatchgrabber is fully opened.
    // If rebooted, the encoder will initialize to zero, but that would be incorrect
    // (and doing an "open" from that open position would break the grabber
    // mechanism)
    // So, if we need to reset the hatchgrabber state, use this function to override
    // the sensor to be the (desired) max hatchgrabber encoder value
    public void forceHatchGraberEncToMax() {
        hatchGrabberMotor.setSelectedSensorPosition((int) kMaxGrabValue); // Now the subsystem will believe the grabber
                                                                          // is fully opened...
    }
    ...
    public void hatchGrabberPIDMove(double magDir) {
        ...
        if ((magDir > kMaxGrabValue) | (magDir < kMinGrabValue)) {
            // Error: we are out of the legal operating range.
            /*
             * System.out.
             * println("Error:  HatchGrabber setpoint is outside of allowable range.\n" +
             * "Range min:max = " + kMinGrabValue + ":" + kMaxGrabValue +
             * ", requested value is = " + magDir);
             */
        } else {
    ...
    // Simple method for moving the hatch motor under "Open loop" control, with
    // magnitude and direction taken directly from Shuffleboard
    ...
    public boolean isReverseLimitPressed() {
        ...
        // motion of the arm causes the encoder to creep up a few ticks. We still
        // consider
        // that to be "closed"
    ...
    public State getState() {
        if (isForwardLimitPressed()) {
            return State.OPENED;
        } else if (isReverseLimitPressed()) {
            return State.CLOSED;
        } else {
    ...
    LaunchPadJoystick m_launchPadJoystick;
