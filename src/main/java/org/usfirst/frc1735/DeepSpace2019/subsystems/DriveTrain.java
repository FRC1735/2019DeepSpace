...
public class DriveTrain extends Subsystem implements PIDOutput {
    ...
    public void periodic() {
        ...
        // SmartDashboard.putNumber("Yaw", Robot.ahrs.getYaw()); //-180 to +180 from
        // last zeroed yaw setting
    ...
    public void drivetrainInit() {
        // Do initialization that cannot be done in the constructor because robot.init
        // isn't executed yet so we don't have a gyro instance.

        // -----------------------------
        // Software PID Subsystem Initialization
        // -----------------------------
        // PID values.values F is feed-forward for maintaining rotational velocity.
        // The last two args are input source and output object (our PIDwrite()
        // function)
        ...
        drivelineController.setOutputRange(-0.75, 0.75); // What is the allowable range of values to send to the output
                                                         // (our motor rotation)
        drivelineController.setAbsoluteTolerance(kToleranceDegrees); // How close do we have to be in order to say we
                                                                     // have reached the target?
        // Robot can spin in full circle so angle might wrap from 0 to 360.
        // 'Continuous' allows us to follow that wrap to get from 359 to 0, rather than
        // going counterclockwise a full circle to get there.
        ...
        leftMotor.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative, 0, 0); // extra args are:
                                                                                               // primary closed loop,
                                                                                               // timeout in ms
        ...
        //
        // See full code example at
        // https://github.com/CrossTheRoadElec/FRC-Examples-STEAMWORKS/blob/master/JAVA_MotionMagicExample/src/org/usfirst/frc/team217/robot/Robot.java
        // General documentation on CTRE examples at
        ...
        leftMotor.configMotionAcceleration(kAccelUnits, 0); // want to get to full speed in 1/3 sec, so triple the
                                                            // velocity
        ...
        /*
         * . SmartDashboard.putNumber("Cruise SpeedDir", 2700); // speed in units per
         * 100ms (2745 is full speed) SmartDashboard.putNumber("Cruise Dist", 72); //
         * inches (DriveWithPID will convert to encoder ticks)
         * SmartDashboard.putNumber("Cruise Accel", kAccelUnits); //8100 = 1/3 sec to
         * get to full 2700 speed SmartDashboard.putNumber("Cruise R Accel",
         * kAccelUnits); //10000 = Temporary to allow setting left vs right accel
         * separately SmartDashboard.putNumber("P", kDistP);
         * SmartDashboard.putNumber("I", kDistI); SmartDashboard.putNumber("D", kDistD);
         * SmartDashboard.putNumber("F", kDistF);
         */

        // Default to be in "drive by joystick" mode
        // if we are going into Autonomous, the first Command will properly set the
        // operating mode so we're still ok
        setOpenLoopMode();

    }

    public void setOpenLoopMode() {
        // This function does the Talon configuration changes to allow operation in an
        // open-loop mode
        // (Which is what we use for joystick control)

        // in open loop mode, the WPI lib handles motor controller inversion, so turn it
        // off in the hardware.
        ...
        // When driving in Open Loop mode via the differentialDrive object, turn on the
        // WPILib safety checks
        differentialDrive1.setSafetyEnabled(false);

    }

    public void setPIDMode() {
        // This function does the Talon configuration changes to allow operation in a
        // closed-loop PID mode.
        // this allows us to use the Talon HW PID for positional control,
        // and if in MotionMagic mode, it will also obey the trapezoidal motion profile
        // for acceleration
        // and maximum velocity.

        // In closed-loop mode we talk to the motor controllers directly, and don't get
        // the inversion of the right-hand motors that's implemented
        // in the WPIlib DifferentialDrive class. So, we have to invert the motor
        // controller hardware directly:
        rightMotor.setInverted(true);
        rightFollower.setInverted(true);

        // Set the mode to Magic (for the master; slaves match for now). Second arg is
        // the setpoint.
    ...
    public void drive(final double joystickAX, final double joystickAY, final double joystickBX,
            final double joystickBY) {
    ...
    // Autonomous Turn command wants to control the driveline directly in tank mode,
    // so expose as public
    ...
    // Expose thes initial compiled-in settings to the SmartDashboard for
    // development work and overrides. We will only sample them once, when enabling
    // the robot.
    // SOME of these values can be different between practice and competition
    // robots, so take that into account for initial settings...
    ...
    // This function samples the PID values from the SmartDashboard and stores them
    // locally.
    // This allows us to change the values from the SmartDashboard, but store them
    // locally for higher performance.
    // Use the current value as the default if SD is broken and doesn't return a
    // value...
    ...
    public void setSmallTurnPID() {
        ...
        drivelineController.setOutputRange(-smallTurnPIDOutputMax, smallTurnPIDOutputMax); // What is the allowable
                                                                                           // range of values to send to
                                                                                           // the output (our motor
                                                                                           // rotation)
    }

    public void setMedTurnPID() {
        drivelineController.setPID(medTurnP, medTurnI, medTurnD, medTurnF);
        drivelineController.setOutputRange(-medTurnPIDOutputMax, medTurnPIDOutputMax); // What is the allowable range of
                                                                                       // values to send to the output
                                                                                       // (our motor rotation)
    }
    ...
    public void setLargeTurnPID() {
        ...
        drivelineController.setOutputRange(-largeTurnPIDOutputMax, largeTurnPIDOutputMax); // What is the allowable
                                                                                           // range of values to send to
                                                                                           // the output (our motor
                                                                                           // rotation)
    }
    ...
    public static boolean kUseCamera = true; // used in DriveWithPID to override distance: Instead, distance is
                                             // calculated via the camera.

    // --------------------------------
    // SW PID subsystem variables
    // --------------------------------
    public PIDController drivelineController; // Use this PID controller to accomplish turns and track straight when
                                              // driving forward.
    ...
    public static final double kEncoderTicksPerInch = (4096 / (3.1415 * 6)); // 4096 encoder ticks per revolution; wheel
                                                                             // diameter is nominally 6"
    static final double kToleranceDegrees = 0.5; // Stop if we are within this many degrees of the setpoint.
    public static final double kToleranceDistUnits = (int) 1/* inches */ * kEncoderTicksPerInch; // stop if we are
                                                                                                 // within this many
                                                                                                 // encoder units of our
                                                                                                 // setpoint. 18.85
                                                                                                 // inches/rev and 4096
                                                                                                 // ticks/rev means .25"
                                                                                                 // is ~50 encoder ticks
    ...
    public static final double kMedTurnPIDLimit = 15.0; // Errors smaller than this number of degrees should use the
                                                        // medium PID profile
    static final double kMedTurnP = 0.2; // use a very large (relative to normal) P to guarantee max motor output
    static final double kPracticeMedTurnP = 0.1; // use a very large (relative to normal) P to guarantee max motor
                                                 // output
    ...
    public static final double kSmallTurnPIDLimit = 1; // Errors smaller than this number of degrees should use the
                                                       // small PID profile
    static final double kSmallTurnP = 0.4; // use a very large (relative to normal) P to guarantee max motor output
    static final double kPracticeSmallTurnP = 0.2; // use a very large (relative to normal) P to guarantee max motor
                                                   // output
    ...
    static final int kAccelUnits = (int) (kTopSpeed / kAccelTime); // velocity to reach after 1 second (2700 is full
                                                                   // speed)
    ...
    // We also need LOCAL COPIES of the following values, which will start as the
    // constants above but can be overridden by the SmartDashboard via
    // updatePIDValuesFromSD()
