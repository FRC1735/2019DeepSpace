...
public class AlienDeployer extends Subsystem {
    ...
    public void periodic() {
        ...
        if (m_timeoutDisabled && isReverseLimitPressed()) {
            DriverStation.reportWarning("Alien was disabled but limit switch indicates it is retracted.  Re-enabling.", false);
    ...
    // The motor is set to "inverted", meaning that sending a 1 causes it to go backwards.
    // This also has the unfortunate side effect of causing a motor =1 ("forwards") to look at the REVERSE limit switch to halt the motor in hardware!
    // So, we swap the switches going into the motor controller so that the forward limit switch is actually wired to the reverse port.
    // We also need to reverse the SOFTWARE view of the switch as well, which we do here in these two functions
    public boolean isForwardLimitPressed() {
        return m_reverseLimit.get(); // yes, we really intend to look at the reverse limit switch value due to the motor inversion.
    }

    public boolean isReverseLimitPressed() {
        return m_forwardLimit.get(); // yes, we really intend to look at the forward limit switch value due to the motor inversion.
    }
    ...
    // perodic for this class can clear the flag if the reverse limit switch is triggered.
