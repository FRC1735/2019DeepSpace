...
public class Arm extends Subsystem {
    ...
    public void periodic() {
        if (!Robot.isPracticeBot()) {
            ...
            // m_absEncoderEntry.setDouble(encoderValue); // Current absolute encoder val.
            // 1024 ticks/revolution = 2.84444
    ...
    public void writePIDsToSD() {
        /*
         * // read selected slot from SD final int selectedSlot = (int)
         * m_slotIndexEntry.getDouble(0);
         * 
         * // get the values for that slot final double p = m_pArm[selectedSlot]; final
         * double i = m_iArm[selectedSlot]; final double d = m_dArm[selectedSlot]; final
         * double iz = m_izArm[selectedSlot]; final double ff = m_ffArm[selectedSlot];
         * 
         * // push them to the SD m_pArmEntry.setDouble(p); m_iArmEntry.setDouble(i);
         * m_dArmEntry.setDouble(d); m_izArmEntry.setDouble(iz);
    ...
    public void updatePIDsFromSD() {
        /*
         * // read PID coefficients from SmartDashboard (Arg is the default) double p =
         * m_pArmEntry.getDouble(0); double i = m_iArmEntry.getDouble(0); double d =
         * m_dArmEntry.getDouble(0); double iz = m_izArmEntry.getDouble(0); double ff =
         * m_ffArmEntry.getDouble(0); double max = m_maxOutputArmEntry.getDouble(0);
         * double min = m_minOutputArmEntry.getDouble(0); int slotIdx = (int)
         * m_slotIndexEntry.getDouble(0);
         * 
         * // if PID coefficients on SmartDashboard are different from our latest
         * operating // values, write the new values to controller and store them
         * locally. if ((p != m_pArm[slotIdx])) {
         * //System.out.println("Setting arm P for slot " + slotIdx);
         * armMotor.config_kP(slotIdx, p, 0); m_pArm[slotIdx] = p; } if ((i !=
         * m_iArm[slotIdx])) { armMotor.config_kI(slotIdx, i, 0); m_iArm[slotIdx] = i; }
         * if ((d != m_dArm[slotIdx])) { armMotor.config_kD(slotIdx, d, 0);
         * m_dArm[slotIdx] = d; } if ((iz != m_izArm[slotIdx])) {
         * armMotor.config_IntegralZone(slotIdx, (int) iz, 0); m_izArm[slotIdx] = iz; }
         * if ((ff != m_ffArm[slotIdx])) { armMotor.config_kF(slotIdx, ff, 0);
         * m_ffArm[slotIdx] = ff; } if ((max != m_maxOutputArm) || (min !=
         * m_minOutputArm)) { armMotor.configPeakOutputForward(max, 0);
         * armMotor.configPeakOutputReverse(min, 0); m_minOutputArm = min;
         * m_maxOutputArm = max; }
         */
    }
    ...
    public void updateSetpointFromSD() {
         ...
         * //System.out.println(setpointDegrees); // if ((setpointDegrees !=
         * m_setpointDegreesArm)) { m_setpointDegreesArm = setpointDegrees;
         * //System.out.println("Changing ARM setpoint to new value: " +
         * m_setpointDegreesArm); armMotor.set(ControlMode.Position,
    ...
    public void initArm() {
        ...
        /*
         * 
         * m_armTab = Shuffleboard.getTab("Arm"); // Creates tab if it doesn't already
         * exist.
         * 
         * m_pArmEntry = m_armTab.add("P Arm", kPArm[0]).withSize(2, 1) // make the
         * widget 2 wide x1 tall .withPosition(0, 0) // col, row .getEntry();
         * m_iArmEntry = m_armTab.add("I Arm", kIArm[0]).withSize(2, 1).withPosition(2,
         * 0).getEntry(); m_dArmEntry = m_armTab.add("D Arm", kDArm[0]).withSize(2,
         * 1).withPosition(4, 0).getEntry(); m_izArmEntry = m_armTab.add("I Zone Arm",
         * kIzArm[0]).withSize(3, 1).withPosition(6, 0).getEntry(); m_ffArmEntry =
         * m_armTab.add("Feed Forward Arm", kFFArm[0]).withSize(4, 1).withPosition(9,
         * 0).getEntry(); m_minOutputArmEntry = m_armTab.add("Min Output Arm",
         * kMinOutputArm).withSize(4, 1).withPosition(0, 2) .getEntry();
         * m_maxOutputArmEntry = m_armTab.add("Max Output Arm",
         * kMaxOutputArm).withSize(4, 1).withPosition(4, 2) .getEntry();
         * m_setpointArmEntry = m_armTab.add("Set Degrees Arm", 0).withSize(4,
         * 1).withPosition(8, 2).getEntry(); m_absEncoderEntry =
         * m_armTab.add("Abs Encoder", armMotor.getSelectedSensorPosition()).withSize(4,
         * 1) .withPosition(8, 4).getEntry(); m_simpleArmJoyvalEntry =
         * m_armTab.add("Simple Arm Joyval", 0).withSize(4, 1).withPosition(8,
         * 8).getEntry();
         * 
         * // Command to trigger a read of all ARM-related variables from Shuffleboard
         * m_armTab.add("UpdateArmPIDs", new UpdateArmPIDs()).withSize(4,
         * 1).withPosition(13, 0); // Command to force an update of the setpoint from
         * Shuffleboard m_armTab.add("UpdateArmSetpoint", new
         * UpdateArmSetpoint()).withSize(4, 1).withPosition(12, 2);
         * m_armTab.add("TareArm", new TareArm()).withSize(4, 1).withPosition(12, 3); //
         * Raw analog value from rotary switch m_rawRotaryValue =
         * m_armTab.add("Raw Rotary Value", 0).withSize(4, 1).withPosition(4,
         * 4).getEntry();
         * 
         * m_slotIndexEntry = m_armTab.add("Slot Index", 0).withSize(4,
         * 1).withPosition(17, 0).getEntry();
         * 
         * m_armDegreesEntry = m_armTab.add("Arm Degrees", 0).withSize(2,
         * 1).withPosition(0, 6).getEntry();
         * 
         * m_selectedSlotEntry = m_armTab.add("Selected Slot", 0).withSize(2,
         * 1).withPosition(2, 6).getEntry();
         * 
         * m_armTab.add("Get PIDs", new WriteArmPIDFromSlotToSD()).withSize(4,
         * 1).withPosition(17, 4);
         */
    }
    ...
    public void simpleMoveArm(double magDir) {
        ...
        // m_simpleArmJoyvalEntry.setDouble(motorVal); // MagDir comes directly from
        // Joysticks (-1 to 1); need to scale to
    ...
    /*
     * // Member variables // Pointer to the Shuffleboard tab for the Arm:
     * ShuffleboardTab m_armTab; // Entries for each of the values we want to
     * manipulate in the tab NetworkTableEntry m_pArmEntry; NetworkTableEntry
     * m_iArmEntry; NetworkTableEntry m_dArmEntry; NetworkTableEntry m_ffArmEntry;
     * NetworkTableEntry m_izArmEntry; NetworkTableEntry m_maxOutputArmEntry;
     * NetworkTableEntry m_minOutputArmEntry; NetworkTableEntry m_setpointArmEntry;
     * // Change this to modify the setpoint (in rotations) NetworkTableEntry
     * m_processVariableArmEntry; // current encoder position (Value? Rotations?)
     * NetworkTableEntry m_simpleArmJoyvalEntry; // to print current joystick value
     * for calibration of motor limits NetworkTableEntry m_absEncoderEntry; // For
     * absolute encoder values NetworkTableEntry m_rawRotaryValue; // for the rotary
     * switch sampled value NetworkTableEntry m_slotIndexEntry; // The selected PID
     * slot NetworkTableEntry m_armDegreesEntry; NetworkTableEntry
     * m_selectedSlotEntry;
     */
    ...
    static final double kMaxOutputArm = 0.6;
    static final double kMinOutputArm = -0.6;
